...
PtFraction FractionCreate (int pnum, int pden)	/* construtor inicializador - initializer constructor */
{
	...
	PtFraction frac;	//create variable with the struct

	//if den is 0 return null and set error to NULL_DEN
	if (pden == 0) {	
		Error = NULL_DEN; 
		return NULL;
	}
	//if den is less than 0 set it to positive  and change the signal of the num
	if (pden < 0) {
		pnum = -pnum;
		pden = -pden;
	}
	//if there's a try to create a new fration and its empty theres memory empty
