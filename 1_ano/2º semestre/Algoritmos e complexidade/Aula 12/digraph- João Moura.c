//João Jorge Oliveria Moura nº mec: 43889 turma P2
...
int VertexType (PtDigraph pdig, unsigned int pv)
{
  /* Insira o seu código - Insert your code */
	...
	PtBiNode tmp = OutPosition(pdig->Head,pv);
	PtVertex vert;
	...
	//Comparaçoes
	vert = (PtVertex) no->PtElem;
	if(vert->InDeg != 0 && vert->OutDeg == 0)
		return SINK;
...
int VertexClassification (PtDigraph pdig, unsigned int pv, double *pclass)
{
  /* Insira o seu código - Insert your code */
	...
	//formula da classificaçao 
	*pclass = (double)(vert->InDeg + vert->OutDeg) / (2 * ((pdig->NVertexes) - 1));
  ...
  /* Insira o seu código - Insert your code */
	...
	while(vert != NULL)
	{
		...
		while(ares != NULL)
		{
			...
			InEdge(tmp, ares->Number, vert->Number, edge->Cost);
			ares = ares->PtNext;
		}
		vert = vert->PtNext;
	}
...
//funçao para descobrir o ultimo vertice do digrafo
...
int DigraphEdgeSplit (PtDigraph pdig, unsigned int pve, unsigned int pvi)
{
  /* Insira o seu código - Insert your code */
	...
	PtEdge edge;
	int cost1, cost2, Err;
	unsigned int last = LastVertexNumber(pdig); //ultimo vertice

	V1 = OutPosition(pdig->Head,pve);
	V2 = OutPosition(pdig->Head,pvi);

	//se nao existir aresta retorna no-edge

	if (V1->PtAdj == NULL || OutPosition (V1->PtAdj, pvi) == NULL)
		return NO_EDGE;	/* aresta inexistente */
	else
		ares = OutPosition (V1->PtAdj, pvi);
	
	
	edge = (PtEdge) ares->PtElem;

		
	//custo de cada aresta nova
	if(edge->Cost % 2 == 0) {
		cost1 = edge->Cost / 2;
		cost2 = cost1;
	}
	...
	OutEdge(pdig, pve, pvi); //elimina a aresta
			

		
	//Inserir vertice novo e inserir as arestas
		
	Err = InVertex(pdig, last+1); //novo vertice a seguir a ultima posiçao, se falhar retorna no_memory
	if(Err == NO_MEM)
		return NO_MEM;
	last =  last +1; //posiçao do novo ultimo vertice

	Err = InEdge(pdig, V1->Number, last, cost1); // cria nova aresta do pve ate novo vertice, se falhar retorna no mem e volta ao estado inicial
	if(Err == NO_MEM) {
		OutVertex(pdig, last);
		return NO_MEM;
	}
	Err = InEdge(pdig, last, V2->Number, cost2); // cria nova aresta do novo vertice ate pvi, se falhar retorna no mem e volta ao estado inicial
	if(Err == NO_MEM){
		OutVertex(pdig, last);
		return NO_MEM;
	}
...
int DigraphVertexSplit (PtDigraph pdig, unsigned int pv)
{
  /* Insira o seu código - Insert your code */
	...
	int Err, cost;
	PtEdge edge;
	PtBiNode V1 = OutPosition(pdig->Head,pv);
	PtBiNode tmp = V1->PtAdj;
	PtBiNode ares, ver;
	ver = pdig->Head;
	unsigned int last = LastVertexNumber(pdig); //ultimo vertice
	...
	if(Err == NO_MEM)
		return NO_MEM;
	last = last + 1; // novo vertice a seguir ao ultimo (vertice 6)
	...
	last = last + 1; //vertice 7

	Err = InEdge(pdig, last-1, last, 0); //nova aresta custo 0 entre os dois vertices, se falha retorna ao estado original
	if(Err == NO_MEM) {
		OutVertex(pdig, last);
		OutVertex(pdig, last-1);
		return NO_MEM;
	}


	//cria arestas emergentes do vertice eliminado no ultimo vertice
	while(tmp!=NULL){
		edge = (PtEdge) tmp->PtElem;
		cost = edge->Cost;
		Err = InEdge(pdig, last, tmp->Number,cost);
		
		if(Err == NO_MEM) { //se falhar retorna ao original
			OutVertex(pdig, last);
			OutVertex(pdig, last-1);
			return NO_MEM;
		}
	...
	//cria arestas incidentes do vertice eliminado no penultimo vertice
	while(ver != NULL) {
		ares = ver->PtAdj;
		while(ares != NULL)
		{
			if(ares->Number == pv) {
				edge = (PtEdge) ares->PtElem;
				cost = edge->Cost;
				Err = InEdge(pdig, ver->Number, last-1,cost);
	...
	OutVertex(pdig,pv); //elimina o vertice pedido
...
int AllSuccDist (PtDigraph pdig, unsigned int pv, double pdst, PtQueue *pqueue)
{
  /* Insira o seu código - Insert your code */
	...
	if(pdig == NULL)
		return NO_DIGRAPH;
	if((V1 = OutPosition(pdig->Head, pv)) == NULL)
		return NO_VERTEX;
	if(pdig->NVertexes == 0)
		return DIGRAPH_EMPTY;
	if(pqueue == NULL)
		return NULL_PTR;
	if((*pqueue =  QueueCreate(sizeof(unsigned int))) == NULL)
		return NO_MEM;

	ares = V1->PtAdj;
	PtVertex vex = (PtVertex)V1->PtElem;
	unsigned int n = vex->OutDeg;
	
	//junta as arestas na fila
	while(n > 0) {
		...
		if(edge->Cost < pdst){
			...
			QueueEnqueue(*pqueue,&ares->Number); 
		}
		n--;
		ares = ares->PtNext;
	}
	...
	while(ver != NULL) {
		...
		while(ares != NULL)
		{
			if(ares->Number == pv) {
				...
				QueueEnqueue(*pqueue, &ver->Number);	
