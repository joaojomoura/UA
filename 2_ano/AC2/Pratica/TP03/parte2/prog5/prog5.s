...
main:
    ...
    li          $s2, 0                  # count = 0
    while:
        andi        $s3, $s2, 1             # extrair bit 0 (mais significativo)
        srl         $s2, $s2, 1             # count = count >> 1
        andi        $s2, $s2, 0xFFF7        # bit 3 = 0

        if1:
            bne		    $s3, 1, if2             # if(bit0 == 1)
            ori         $s2, $s2, 0             #   count[bit3] = 0
            j           loop
        if2:
            ori         $s2, $s2, 8             #   count[bit3] = 1
        loop:
