...
public class VideoClube {
	...
	public void addsoc()
	{
		...
		if (alORfu == 1)
		{
			...
			lista.putFunc(new Funcionario(nm, cc, new Data(dOi[0], dOi[1], dOi[2]), new Data(dOf[0], dOf[1], dOf[2]), nFunc, nFisc, maxLoans));
		}
		else
		{
			k.nextLine();
			...
			lista.putAluno(new Aluno(nm, cc, new Data(dOi[0], dOi[1], dOi[2]), new Data(dOf[0], dOf[1], dOf[2]),nmec, course, maxLoans));
		}
	...
	public void remsoc()
	{
		int alORfu;
		do {
			out.printf("Aluno(0) ou Funcionário(1): ");
			alORfu = k.nextInt();
		}while(alORfu != 0 && alORfu != 1);
		...
		{
			lista.remFunc(ns);
			listaEmprestimos.remEmprestimosOfSoc(ns, listaFilmes.movieAmount());
	...
	public void listVid()
	{
		...
		try
		{
			if (alORfu == 1)
			{
				...
				int idade = (Year.now().getValue()) - (tmp.getNascimento()).getAno();
				int m = Calendar.getInstance().get(Calendar.MONTH) + 1 - (tmp.getNascimento()).getMes();
				int d = Calendar.getInstance().get(Calendar.DAY_OF_MONTH) - (tmp.getNascimento()).getDia();
				if (m < 0 || d < 0)
				{
					idade--;
				}
