...
class SemanticNetwork:
    ...
    def list_associations(self):
        d  = list(set([d.relation.name for d in self.declarations if isinstance(d.relation, Association)]))
        print(d)
        return d
    ...
    def query_local_assoc(self, ent, assoc):
        ...
        for d in self.declarations:
            if d.relation.entity1 == ent and d.relation.name == assoc:
                # if the object d.relation is an instance of AsscOne
                if isinstance(d.relation, AssocOne):
                    # if the entity2 is in dicAssocOne
                    if d.relation.entity2 in dicAssocOne:
                        # plus one for each entity2 equal
                        dicAssocOne[d.relation.entity2]+=1
                    # else gets the value 1
                    else:
                        dicAssocOne[d.relation.entity2]=1
                # if is AsscoNum
