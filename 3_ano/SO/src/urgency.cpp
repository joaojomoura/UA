...
void init_simulation(uint32_t np, uint32_t nn, uint32_t nd) {
  ...
  hd->doctor_thread = new pthread_t[nd];
  hd->nurse_thread = new pthread_t[nn];
  hd->patient_thread = new pthread_t[np];
  hd->patient_index = new uint32_t[np];
  ...
  for (uint32_t i = 0; i < nd; i++) {
    ...
    thread_create(&hd->doctor_thread[i], NULL, doctorThread, NULL);
  }
  ...
  for (uint32_t i = 0; i < nn; i++) {
    ...
    thread_create(&hd->nurse_thread[i], NULL, nurseThread, NULL);
  }
  ...
  for (uint32_t i = 0; i < np; i++) {
    ...
    hd->patient_index[i] = i;
    thread_create(&hd->patient_thread[i], NULL, patientThread,
                  &hd->patient_index[i]);
  }

  /* Waiting for all patient threads to finish */
  for (uint32_t i = 0; i < np; i++) {
    thread_join(hd->patient_thread[i], NULL);
  ...
  /* Waiting for all nurse threads to finish */
  for (uint32_t i = 0; i < nn; i++) {
    thread_join(hd->nurse_thread[i], NULL);
    printf("Nurse thread %d has terminated\n", i);
  }

  /* Waiting for all doctor threads to finish */
  for (uint32_t i = 0; i < nd; i++) {
    thread_join(hd->doctor_thread[i], NULL);
...
int nurse_iteration() {
  ...
  printf(
      "\e[34;01mNurse: add patient %u with priority %u to doctor queue\e[0m\n",
      patient, priority);
...
void patient_wait_end_of_consultation(int id) {
  ...
  // if patient consultation not done, wait
...
/* ************************************************* */
void *patientThread(void *args) {
  uint32_t *npatients = (uint32_t *)args;
  patient_life(*npatients);
  return NULL;
}
