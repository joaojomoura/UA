...
void *consumer(void *argp) {
  ...
  for (i = 0; i < argv->niter; i++) {
    ...
    if (value == 99999999 || pid == 99999999 || (value % 100) != pid)
      printf("\e[31;01mThe value %05u was produced by thread P%u and consumed "
             "by thread C%u!\e[0m\n",
             value, pid, argv->id);
    else
      printf("\e[34;01mThe value %05u was produced by thread P%u and consumed "
             "by thread C%u!\e[0m\n",
             value, pid, argv->id);
  }
...
/*   main thread: it starts the simulation and generates the producer and
 * consumer threads */
